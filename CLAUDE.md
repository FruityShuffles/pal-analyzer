...
- Run `build_id_maps.py` only when refreshing the internal-ID maps (same cadence as `build_data.py`). Source is the palworld-save-pal project (distinct from the wiki — keep its attribution); it also emits the per-species `friendship` map used by Trust scoring. Then `ingest_save.py` reads `Level.sav` + the `<UID>.sav` player saves in the folder and writes `out/`. Its `ingest_review.md` flags any species/passive present in the save but missing from the wiki data — triage combat-relevant passives into `PASSIVE_OVERRIDES` in `build_data.py`. **The `out/*.json` files hold real players' Pal rosters — treat them like `pal_analyzer.html` (never publish as an Artifact).**
...
- **HP has no passive** in the wiki data, so `hp_pct` is always 0 — kept only for schema symmetry. HP still has an outer multiplier stage for Souls and Condensing.
...
- **`data/passives.json`** — `{ passive: { attack_pct, defense_pct, hp_pct(=0), rank, description } }`. Only `stat=="Attack"`/`"Defense"` percent rows affect scoring; all other stat rows (Work Speed, Element Boost/Resist, etc.) are ignored but the full passive list is kept so the picker can offer every real passive name.
